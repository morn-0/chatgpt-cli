...
pub fn decrypt(data: Vec<u8>, key: &str) -> Result<String> {
    let dec_data = ase_decrypt(data, key)?;
    Ok(String::from_utf8(dec_data)?)
}

fn aes_encrypt(content: &str, password: &str) -> Result<EncryptionData> {
    let salt = rand::random::<[u8; 8]>().to_vec();
    #[rustfmt::skip]
    let (key, iv) = default_evp_kdf(password.as_bytes(), &salt).map_err(|err| anyhow::anyhow!(err))?;
...
fn ase_decrypt(content: Vec<u8>, password: &str) -> Result<Vec<u8>> {
